...
def get_datasets(imgs_dir,groundTruth_dir,borderMasks_dir,train_test="null"):
    ...
    for path, subdirs, files in os.walk(imgs_dir): #list all files, directories in the path
        for i in range(len(files)):
            #original
            print("original image: {}".format(files[i]))
            img = Image.open(imgs_dir+files[i])
            imgs[i] = np.asarray(img)
            #corresponding ground truth
            groundTruth_name = files[i][0:2] + "_manual1.gif"
            print("ground truth name: {}".format(groundTruth_name))
            ...
            if train_test=="train":
                ...
                exit()
            print("border masks name: {}".format(border_masks_name))
            b_mask = Image.open(borderMasks_dir + border_masks_name)
            border_masks[i] = np.asarray(b_mask)

    print("imgs max: {}".format(str(np.max(imgs))))
    print("imgs min: {}".format(str(np.min(imgs))))
